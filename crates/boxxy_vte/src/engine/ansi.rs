...
//
// This module was originally part of an upstream project, which is
...
use crate::engine::vte::{Params, ParamsIter};
...
pub struct Processor<T: Timeout = StdSyncHandler> {
    ...
    parser: crate::engine::vte::Parser,
}
...
pub trait Handler {
    /// OSC to set current working directory (OSC 7).
    ...
    fn identify_terminal(&mut self, _intermediate: Option<char>, _action: char) {}
...
/// [`output`]: https://invisible-island.net/xterm/modified-keys.html
...
pub enum ModifyOtherKeys {
    ...
    ///
    /// [`example`]: https://invisible-island.net/xterm/modified-keys.html
...
impl<'a, H, T> crate::engine::vte::Perform for Performer<'a, H, T>
where
    H: Handler + 'a,
    T: Timeout,
{
    ...
    fn apc_dispatch(&mut self, data: &[u8]) {
        ...
        if data[0] == b'G' {
            ...
            debug!("[unhandled] apc_dispatch len={}", data.len());
        }
    }
