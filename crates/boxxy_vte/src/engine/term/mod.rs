...
pub struct RenderState {
    ...
    pub kitty_placements: Vec<crate::engine::kitty::Placement>,
    pub kitty_images: HashMap<u32, Arc<crate::engine::kitty::KittyImage>>,
    pub cell_width: u16,
    pub cell_height: u16,
    ...
    pub fn new<T>(term: &Term<T>) -> Self {
        ...
        for row in 0..screen_lines {
            for col in 0..columns {
                ...
                cells.push(grid[point.line][point.column].clone());
            }
        }
        ...
        Self {
            ...
            selection_range,
            kitty_placements: term.kitty_graphics.placements.clone(),
            kitty_images: term.kitty_graphics.images.clone(),
...
pub struct Term<T> {
    ...
    pub kitty_graphics: crate::engine::kitty::KittyGraphics,
...
impl<T> Term<T> {
    ...
    pub fn resize<S: Dimensions>(&mut self, size: S) {
        ...
        self.kitty_graphics.rotate_placements(&range, -delta);
...
impl<T: EventListener> Handler for Term<T> {
    ...
    #[inline]
    fn identify_terminal(&mut self, intermediate: Option<char>, action: char) {
        match (action, intermediate) {
            ('c', None) | ('Z', None) => {
                trace!("Reporting primary device attributes");
                // kitten icat expects the payload to be > 3 bytes, so `?6c` fails.
        ...
        if let Some((cols, rows, width, height)) = cursor_offset {
            ...
            let cols = cols.unwrap_or_else(|| {
                if self.cell_width > 0 {
                    (width as f32 / self.cell_width as f32).ceil() as u32
                } else {
                    1
                }
            });
            let rows = rows.unwrap_or_else(|| {
                if self.cell_height > 0 {
                    (height as f32 / self.cell_height as f32).ceil() as u32
                } else {
                    0
                }
            });

            let current_col = self.grid.cursor.point.column.0;
            let current_line = self.grid.cursor.point.line;
...
///
/// This returns the current terminal version as a unique number based on boxxy_vte's
...
pub mod test {
    ...
    ///
    /// ```rust
    /// use boxxy_vte::engine::term::test::mock_term;
    ///
...
mod tests {
    ...
    fn grow_lines_bottom_pinned() {
        ...
        // All 10 lines of history are pulled into view.
    ...
    fn shrink_lines_updates_active_cursor_pos() {
        ...
        // Since the lines were empty, they are truncated instead of pushed to history.
        assert_eq!(term.history_size(), 10);
    ...
    // VTE-level resize integration tests.
    //
    // These tests feed characters through the actual Term/VTE input handler (not
    // the grid directly) and then check that the content is preserved through a
    // column or combined resize.

    #[test]
    fn vte_grow_columns_preserves_content() {
        let mut size = TermSize::new(80, 24);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        // Write "ls" at Line(0), Column(0..1).
        term.input('l');
        term.input('s');
        assert_eq!(term.grid[Line(0)][Column(0)].c, 'l');
        assert_eq!(term.grid[Line(0)][Column(1)].c, 's');
        // Grow columns.
        size.columns = 100;
        term.resize(size);
        assert_eq!(
    ...
    #[test]
    fn vte_shrink_columns_preserves_content() {
        let mut size = TermSize::new(80, 24);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        term.input('l');
        term.input('s');
    ...
    #[test]
    fn vte_combined_resize_preserves_content_at_bottom() {
        // Simulate a terminal where "ls" was typed at the bottom of the screen.
        let mut size = TermSize::new(80, 24);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        // Scroll to fill the terminal so the cursor is at the bottom line.
        for _ in 0..23 {
            term.newline();
        }
        assert_eq!(term.grid.cursor.point.line, Line(23));
        // Write "ls" at the bottom line.
        term.input('l');
        term.input('s');
        assert_eq!(term.grid[Line(23)][Column(0)].c, 'l');
        assert_eq!(term.grid[Line(23)][Column(1)].c, 's');
        // Combined resize: grow both lines and columns.
        size.screen_lines = 28;
        size.columns = 100;
        term.resize(size);
        // "ls" must be somewhere in the new viewport.
        let found = (0..28).any(|r| {
            term.grid[Line(r)][Column(0)].c == 'l' && term.grid[Line(r)][Column(1)].c == 's'
        });
        assert!(found, "'ls' must be visible after combined grow");
    }

    #[test]
    fn vte_combined_shrink_preserves_content_at_bottom() {
        // Terminal with content at the bottom and blank trailing rows.
        let mut size = TermSize::new(80, 24);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        // Scroll to line 20 (leaves 3 blank lines below).
        for _ in 0..20 {
            term.newline();
        }
        term.input('l');
        term.input('s');
        assert_eq!(term.grid[Line(20)][Column(0)].c, 'l');
        assert_eq!(term.grid[Line(20)][Column(1)].c, 's');
        // Combined shrink: fewer lines (trim blanks) and fewer columns.
        size.screen_lines = 22;
        size.columns = 60;
        term.resize(size);
        let found = (0..22).any(|r| {
            term.grid[Line(r)][Column(0)].c == 'l' && term.grid[Line(r)][Column(1)].c == 's'
        });
