...
use crate::engine::grid::{BidirectionalIterator, Dimensions, GridIterator, Indexed};
use crate::engine::index::{Boundary, Column, Direction, Point, Side};
...
impl RegexSearch {
    ...
    pub fn new(search: &str, case_insensitive: bool) -> Result<RegexSearch, Box<BuildError>> {
        debug!("RegexSearch::new: query='{}', case_insensitive={}", search, case_insensitive);
        ...
        let syntax_config = SyntaxConfig::new()
            ...
            .utf8(true);
        let config =
            Config::new().minimum_cache_clear_count(Some(3)).minimum_bytes_per_state(Some(10)).unicode_word_boundary(true);
...
impl<T> Term<T> {
    ...
    fn regex_search_internal(
        ...
        let mut start_config = start::Config::new().anchored(regex_anchored);

        let look_behind_point = match regex.direction {
            Direction::Right => start.sub(self, Boundary::Grid, 1),
            Direction::Left => start.add(self, Boundary::Grid, 1),
        };

        let prev_cell = &self.grid[look_behind_point.line][look_behind_point.column];
        let mut buf = [0; 4];
        let bytes = prev_cell.c.encode_utf8(&mut buf).as_bytes();
        let lb = match regex.direction {
            Direction::Right => bytes[bytes.len() - 1],
            Direction::Left => bytes[0],
        };
        start_config = start_config.look_behind(Some(lb));

        let mut state = regex.dfa.start_state(&mut regex.cache, &start_config)?;
        ...
        macro_rules! reset_state {
            () => {{
                state = regex.dfa.start_state(&mut regex.cache, &start_config)?;
                consumed_bytes = 0;
                regex_match = None;
            }};
        }
...
mod tests {
    ...
    use crate::engine::term::Config;
    use crate::engine::term::test::{TermSize, mock_term};
    ...
    fn regex_right() {
        ...
        let mut regex = RegexSearch::new("Term.*123", false).unwrap();
    ...
    fn nested_regex() {
        #[rustfmt::skip]
        let term = mock_term("\
            Aaa -> Aaabbbbcc -> bbbbcc\r\n\
            bbbbcc\
        ");

        // Greedy stopped at linebreak.
        let mut regex = RegexSearch::new("Aaa.*bbbbcc", false).unwrap();
        ...
        let mut regex = RegexSearch::new("Aaa[^c]*bbbbcc", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn no_match_right() {
        ...
        let mut regex = RegexSearch::new("nothing", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn include_linebreak_left() {
        ...
        let mut regex = RegexSearch::new("te.*123", false).unwrap();
        let start = Point::new(Line(1), Column(0));
        let end = Point::new(Line(0), Column(0));
    ...
    fn skip_dead_cell() {
        ...
        let mut regex = RegexSearch::new("termina", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn reverse_search_dead_recovery() {
        ...
        let mut regex = RegexSearch::new("zoo", false).unwrap();
    ...
    fn multibyte_unicode() {
        ...
        let mut regex = RegexSearch::new("te.*ing", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn end_on_multibyte_unicode() {
        ...
        let mut regex = RegexSearch::new("te.*и", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn fullwidth() {
        ...
        let mut regex = RegexSearch::new("[^ ]*", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn end_on_fullwidth() {
        ...
        let mut regex = RegexSearch::new("x", false).unwrap();
        assert_eq!(term.regex_search_right(&mut regex, start, end), None);
        ...
        let mut regex = RegexSearch::new("jarr🦇", false).unwrap();
        assert_eq!(term.regex_search_right(&mut regex, start, end), Some(start..=match_end));
    }
    ...
    fn wrapping() {
        ...
        let mut regex = RegexSearch::new("xxx", false).unwrap();
        let start = Point::new(Line(0), Column(2));
    ...
    fn multiline() {
        ...
        let mut regex = RegexSearch::new(PATTERN, false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn wide_without_spacer() {
        ...
        let mut regex = RegexSearch::new("test", false).unwrap();
    ...
    fn wrap_around_to_another_end() {
        ...
        // Bottom to top.
        let mut regex = RegexSearch::new("abc", false).unwrap();
        let start = Point::new(Line(1), Column(0));
        ...
        // Top to bottom.
        let mut regex = RegexSearch::new("def", false).unwrap();
        let start = Point::new(Line(0), Column(2));
        let end = Point::new(Line(1), Column(0));
        let match_start = Point::new(Line(1), Column(0));
        let match_end = Point::new(Line(1), Column(2));
    ...
    fn nfa_compile_error() {
        assert!(RegexSearch::new("[0-9A-Za-z]{9999999}", false).is_err());
    }
    ...
    fn runtime_cache_error() {
        ...
        let mut regex = RegexSearch::new("[0-9A-Za-z]{9999}", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn greed_is_good() {
        ...
        // Bottom to top.
        let mut regex = RegexSearch::new("/github.com|https://github.com", false).unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    fn anchored_empty() {
        ...
        // Bottom to top.
        let mut regex = RegexSearch::new(";*|rust", false).unwrap();
        let start = Point::new(Line(0), Column(0));
        let end = Point::new(Line(0), Column(3));
        assert_eq!(term.regex_search_right(&mut regex, start, end), Some(start..=end));
    }
    ...
    fn inline_word_search() {
        ...
        let mut regex = RegexSearch::new("word", false).unwrap();
