...
use crate::engine::tty::{ChildEvent, EventedPty, EventedReadWrite, Options};
...
pub fn from_fd(config: &Options, window_id: u64, master: OwnedFd, slave: OwnedFd) -> Result<Pty> {
    ...
    builder.env("BOXXY_WINDOW_ID", &window_id);
...
impl ToWinsize for WindowSize {
    fn to_winsize(self) -> Winsize {
        ...
        let ws_xpixel = self.pixel_width as libc::c_ushort;
        let ws_ypixel = self.pixel_height as libc::c_ushort;
