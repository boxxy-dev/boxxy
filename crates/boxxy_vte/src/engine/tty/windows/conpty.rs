...
use crate::engine::event::{OnResize, WindowSize};
use crate::engine::tty::Options;
use crate::engine::tty::windows::blocking::{UnblockedReader, UnblockedWriter};
use crate::engine::tty::windows::child::ChildExitWatcher;
use crate::engine::tty::windows::{Pty, cmdline, win32_string};

const PIPE_CAPACITY: usize = crate::engine::event_loop::READ_BUFFER_SIZE;
...
/// the directory where the terminal executable is located.
...
pub fn new(config: &Options, window_size: WindowSize) -> Result<Pty> {
    ...
    // PTY process does not inherit any handles from this process.
