...
- **`engine/session.rs`**: Implements the `ClawSession` actor. Each terminal pane owns its own dedicated session loop and tracks local capability states.
  - **Interrupt & Replace (Human-in-the-Loop):** The engine tracks the `current_turn` via a `tokio::task::JoinHandle`. If the user sends a new message or query while the agent is busy, the engine instantly `.abort()`s the ongoing LLM task, clears the queue, and pivots to the new instruction. This provides a snappy, responsive UX and prevents parallel execution races.
  - **Sequential Peer Queueing:** If a peer agent delegates a task while the target agent is busy, the task is safely pushed to a FIFO `backlog` queue. The agent processes the queue sequentially, ensuring reliable swarm coordination without dropping tasks or corrupting files.
  - **Autonomous Suspension (Sleep Mode):** Agents can pause their reasoning loop while waiting for peer events or sub-task results (consuming 0 tokens). Wakeups are triggered reactively via the internal `EventBus`.
