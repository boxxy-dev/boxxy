...
2.  **Environment Leaking:** When running in a Flatpak, `std::env::vars()` returns the heavily modified, sandboxed environment (e.g., `HOME=/app`). If this environment is passed to the host agent via `SpawnOptions`, the shell will initialize using sandbox paths, breaking user configurations (like `~/.config/fish/config.fish`). To fix this, the UI explicitly passes an empty environment vector (`Vec::new()`) to the agent when in Flatpak mode, instructing the host shell to inherit the true host session environment.
