...
- **Memory Consolidation ("Dreaming")**: A background orchestration pipeline that combats context bloat using an OpenClaw-inspired reinforcement model. Lives on the daemon side so it outlives the UI and can be safety-gated.
  - **Phase 1 (Light Sleep):** Ingests raw interaction logs. Extracts new facts with an LLM-assigned `confidence_score` and increments an `observation_count` for existing facts. All new facts are initially stored in the `candidate` category (`verified: false`).
